...
class Agent(Player):
    ...
    def update_q_table(self, state_idx, new_state_idx, reward, done, action):
        ...
        if not done:
            self._q_table[state_idx][action] = self._q_table[state_idx][action] * (1 - self._learning_rate) + self._learning_rate * (
                    reward + self._discount_rate * env.nanargmax_unbiased(self._q_table[new_state_idx]))
        else:
